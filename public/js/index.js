...
if (query){
	...
	$.ajax({
		...
		for(var i = 0; i < documents.length; i++) {
			...
			document.getElementById("resultsHeader").innerHTML = '<div class="col-xs-6" style="text-align: left; font-family: Helvetica, serif; font-size: 1.8em; "><div style="padding-left:20px;">Displaying '+ documents.length + ' ' + reportWord + '</div></div>' + '<div class="col-xs-6" style="text-align:right; font-family: Helvetica, serif; font-size: 1.8em;"> <!--Sort By Drop-down menu--><div style="padding-right:20px;"><img src="/img/SortBar.png"/> &nbsp Sort by &nbsp <button class="btn btn-default dropdown-toggle" data-toggle="dropdown" id="dropdownSortMenu" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"><span id="sortText">Relevance</span> <img src="/img/DropdownArrow.png"/></button><ul class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownSortMenu"><li><a onclick=\'sortDocuments("Relevance");\'>Relevance</a></li><li><a onclick=\'sortDocuments("Title");\'>Title</a></li><li><a onclick=\'sortDocuments("Title (descending)");\'>Title (descending)</a></li><li><a onclick=\'sortDocuments("Date");\'>Date</a></li><li><a onclick=\'sortDocuments("Date (oldest first)");\'>Date (oldest first)</a></li></ul></div></div></div>'
			displayDocuments();
		});
};
